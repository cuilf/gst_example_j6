...
//gst-launch-1.0 -e v4l2src device=/dev/video1 ! 'video/x-raw, format=(string)UYVY' ! videoconvert ! 'video/x-raw, format=(string)NV12' !  ducatih264enc bitrate=4000 ! queue ! h264parse ! rtph264pay pt=96  ! udpsink host=192.168.2.101 port=5000
...
main (int argc, char *argv[])
{
	...
	GstElement *pipeline, *video_source, *video_convert, *video_enc, *queue, *video_parse, *qtmux, *file_sink;
	GstBus *bus;  
    GstMessage *msg;
	...
	if (!pipeline || !video_source || !video_convert || !video_enc || !queue || !video_parse || !qtmux || !file_sink)
	{
	...
	gst_bin_add_many (GST_BIN(pipeline), video_source, video_convert, video_enc, queue, video_parse, qtmux, file_sink, NULL);	
	
	if (gst_element_link_many (video_enc, queue, video_parse, qtmux, file_sink, NULL) != TRUE)
	{
